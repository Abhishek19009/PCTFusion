        ...
        while True:
            ...
            if last_min > num_votes:
                ...
                for i, file_path in enumerate(test_loader.dataset.files):
                    ...
                    test_name = join(test_path, 'gender_predictions', cloud_name)
                    write_ply(test_name,
                              [points, preds],
                              ['x', 'y', 'z', 'preds'])
...
#                                   ['x', 'y', 'z'] + prob_names)
                    ...
                    pot_name = join(test_path, 'gender_potentials', cloud_name)
...
#                 for seq_ind, seq in enumerate(test_loader.dataset.sequences):
#                     seq_frames = len(test_loader.dataset.frames[seq_ind])
#                     for frame in range(seq_frames):
#                         print(f"Saving Sequence:: {seq} Frame:: {frame}")
#                         frame_preds = np.expand_dims(all_f_preds[seq_ind][frame], axis=-1)
#                         filename = '{}_{:07d}.npy'.format(seq, frame)
#                         filepath = join(test_path, 'test_preds', filename)
